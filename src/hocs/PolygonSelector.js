...
export const methods = {
  ...
  onClick (annotation, e) {
    ...
    for(let a of imageData){
      if(intersects(coordOfClick, a.geometry)){
        //BUSCANDO PONTO MAIS PRÓXIMO
        let distance = Infinity;
        let index = 0;
        if(a.data.classId){
          activeClass = a.data.classId;
        }        
        mapzer_newtreinamento_edit.setAnnotationHoveredStatus(a.data.id, true, true);
        for(let pointIdx in a.geometry.points){
          let d = distanceBetweenPoints(coordOfClick, a.geometry.points[pointIdx]);
          if(d <= distance){
            distance = d;
            index = pointIdx;
          }
        }
        //VERIFICANDO SE É PARA ADICIONAR OU REMOVER
        if(e.ctrlKey){          
          a.geometry.points.splice(index, 1);
        } else {
          a.geometry.points.splice(index, 0, coordOfClick);
        }
        //REORGANIZANDO ORDEM DO ARRAY
        for(let i = 0; i <= index; i++)
        {
          a.geometry.points.push(a.geometry.points.splice(0,1)[0]);
        }
        return {
          ...a,
          geometry: {
